...
const PROXIMITY_REFRESH_INTERVAL = 5000; // 5 seconds - refresh interval in milliseconds
...
const Homepage = ({ onShowAuth }) => {
  ...
  const updateEncounteredIssues = (currentLocation) => {
    ...
    setEncounteredIssues(prevEncountered => {
      ...
      const newIssues = currentNearbyIssues.filter(issue => !encounteredIds.has(issue.id));
      
      if (newIssues.length > 0) {
        console.log(`📌 Added ${newIssues.length} new issues to encountered list`);
      }
  ...
  // Fetch issues from Flask API via Rust backend
  useEffect(() => {
    const fetchIssues = async () => {
      try {
        console.log('🔄 Fetching issues from Flask API...');
        const fetchedIssues = await invoke('fetch_issues_from_flask');
        console.log('✅ Successfully fetched issues:', fetchedIssues);
        setIssues(fetchedIssues);
      } catch (error) {
        console.error('❌ Failed to fetch issues from Flask API:', error);
  ...
  const handleVouchForIssue = async (issueId) => {
    try {
      ...
      if (!authToken) {
        alert('❌ You must be logged in to vouch for issues. Please login first.');
        if (onShowAuth) {
          onShowAuth();
        }
        return;
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      // Trigger map resize
      setMapResizeTrigger(prev => prev + 1);
    }, isDragging ? 0 : 300); // Immediate during drag, delayed after drag ends
    ...
    <div className="homepage">
      ...
      >
        ...
        <div className="search-section">
          <button className="raise-ticket-btn" onClick={handleRaiseIssue}>
            {user ? 'Raise a New Issue' : 'Login to Raise Issue'}
          </button>
        </div>
        ...
        <div className="issues-section">
          {loading ? (
            ...
            const currentNearbyIssues = userLocation ? issues.filter(issue => {
              if (!issue.latitude || !issue.longitude) return false;
              const distance = calculateDistance(
            ...
            return displayIssues.length > 0 ? (
              <>
                ...
                <div className="proximity-info" style={{
                  ...
                  {userLocation ? (
                    <div>
                      <div>{currentNearbyCount} issues in your surroundings</div>
                      {locationLastUpdated && (
                        <div style={{ fontSize: '10px', opacity: 0.7, marginTop: '2px' }}>
                          Location updated: {locationLastUpdated.toLocaleTimeString()}
                        </div>
                      )}
                    </div>
                  ) : (
                    <span>Getting location... - showing all issues for now</span>
                  )}
                </div>
                  ...
                  <div key={issue.id} className="issue-card-simple">
                    ...
                    >
                      <div className="issue-card-main">
                        <h4 className="issue-title-clean">{issue.title}</h4>
                        <span className="issue-date-clean">{formatDate(issue)}</span>
                      </div>
                    </div>
                    <button 
                      className={`vouch-btn-clean ${vouchedIssues.has(issue.id) ? 'vouched' : ''}`}
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      title={vouchedIssues.has(issue.id) ? "You have already vouched for this issue" : "Vouch for this issue"}
                    >
              ...
              <div className="no-issues">
                {userLocation 
                  ? `No civic issues found within ${PROXIMITY_RADIUS}m. Move around to discover nearby issues!`
      ...
      <div className="bottom-icons fixed-navbar">
        <button className="icon-btn" onClick={() => setCurrentView('home')}><FaHome  /></button>
        <button className="icon-btn"><MdReport /></button>
        <button className="icon-btn" onClick={handleViewEmergencyContacts}><IoIosCall /></button>
        {user ? (
          <button className="icon-btn" onClick={() => setCurrentView('account')} title="Account">
            <FaGear />
          </button>
        ) : (
        ...
        <div className="issue-popup-overlay" onClick={closeIssuePopup}>
          <div className="issue-popup-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="issue-popup-body">
              <div className="issue-popup-meta">
                <div className="popup-meta-item">
                  ...
                  <span className="popup-meta-value">{formatDate(popupIssue)}</span>
                </div>
                <div className="popup-meta-item">
                  <span className="popup-meta-label">Status:</span>
                  <span className={`popup-status status-${popupIssue.status.toLowerCase().replace(' ', '-')}`}>
                    {popupIssue.status}
                  </span>
                </div>
                {popupIssue.category && (
                  <div className="popup-meta-item">
                    ...
                    <span className="popup-meta-value">{popupIssue.category}</span>
                  </div>
                )}
                {popupIssue.priority && (
                  <div className="popup-meta-item">
                    <span className="popup-meta-label">Priority:</span>
                    <span className={`popup-meta-value priority-${popupIssue.priority.toLowerCase()}`}>
            ...
            <div className="issue-popup-footer">
              <button 
                className={`popup-vouch-btn ${vouchedIssues.has(popupIssue.id) ? 'vouched' : ''}`}
                onClick={() => {
                ...
                title={vouchedIssues.has(popupIssue.id) ? "You have already vouched for this issue" : "Vouch for this issue"}
              >
                {vouchedIssues.has(popupIssue.id) ? '✓ Already Vouched' : 'Vouch for this Issue'}
              </button>
              <button 
      ...
      {/* Issue Form View */}
      <div 
        className={`issue-form-view ${currentView === 'issue-form' ? 'active' : 'hidden'}`}
      >
        <IssueForm 
      ...
      <div className="bottom-icons fixed-navbar">
        <button className="icon-btn" onClick={handleBackToHome}><FaHome  /></button>
        <button className="icon-btn" onClick={handleViewIssuesList}><MdReport /></button>
        <button className="icon-btn" onClick={handleViewEmergencyContacts}><IoIosCall /></button>
        {user ? (
          <button className="icon-btn" onClick={() => setCurrentView('account')} title="Account Settings">
            <FaGear />
          </button>
        ) : (
          <button className="icon-btn" onClick={onShowAuth} title="Login to Access Account">
            <FaGear />
          </button>
        )}
      </div>
      ...
      {showEmergencyContacts && (
        <div className="emergency-modal-overlay" onClick={toggleEmergencyContacts}>
          <div className="emergency-modal" onClick={(e) => e.stopPropagation()}>
            <div className="emergency-header">
              <h2>🚨 Emergency Contacts</h2>
              <button className="close-btn" onClick={toggleEmergencyContacts}>✕</button>
            </div>
            <div className="emergency-content">
              ...
              <div className="emergency-section">
                ...
                <div className="office-info">
                  <p><strong>Address:</strong> N.R. Square, Bengaluru, Karnataka 560002, India</p>
                  ...
                  <p><strong>Email:</strong> comm@bbmp.gov.in</p>
                </div>
              </div>
